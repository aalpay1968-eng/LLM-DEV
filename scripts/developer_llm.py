...
def developer_client_olustur(memory_bank, developer_apis, prompt_fn):
    ...
    for cfg in developer_apis:
        ...
        try:
            ...
            print(f"[OK] Developer LLM: {cfg['name']}")
            return client, cfg["model"], cfg.get("extra_params", {}), sistem_prompt
        except Exception as e:
            print(f"[WARN] {cfg['name']}: {e}")
    raise RuntimeError("Tum Developer LLM API'leri basarisiz.")
...
def developer_llm_cagir(client, model_adi, sistem, kullanici,
    ...
    for deneme in range(maks_deneme):
        ...
        try:
            ...
            print(f"[WARN] Developer LLM deneme {deneme+1}/{maks_deneme}: {e}")
            time.sleep(5)
    return {}
...
def developer_llm_tasarla(client, model_adi, sistem, extra_params,
    ...
    sablonu = {
        ...
        "asama": asama,
        "thought_process": "ReAct: Onceki sonuclari analiz et...",
        ...
        "max_steps": 100,
        "r_rank": 32,
        "beklenen_gelisme": "hangi metrikte iyilesme",
    }
    kullanici = (
        f"Mevcut egitim asamasi: {asama}\n"
        f"Son 3 deney: {json.dumps(gecmis[-3:], indent=2, ensure_ascii=False)}\n\n"
        f"ReAct deseni ile dusun, sonra bu sablonu doldur:\n"
    ...
    sonuc = developer_llm_cagir(client, model_adi, sistem, kullanici,
                                 ...
                                 iz_adi="tasarla")
    return sonuc if sonuc else {**sablonu, "degisiklik_aciklama": "varsayilan config"}
...
def developer_llm_yargic(client, model_adi, sistem, extra_params,
    ...
    rubrik = {
        "thought_process": "Adim adim analiz...",
        "mantik_tutarli_a": True,
        "mantik_tutarli_b": True,
        "dil_karisikligi_a": False,
        "dil_karisikligi_b": False,
        "tercih": "A",
        "red_sebebi": "varsa acikla",
    }
    kullanici = (
        f"Soru:\n{prompt}\n\n"
        f"Yanit A:\n{yanit_a}\n\nYanit B:\n{yanit_b}\n\n"
        "Degerlendirme kriterleri: mantiksal tutarlilik, dil karisikligi yok, "
        "dongusuz akil yurutme, dogru format.\n"
        "[WARN] Uzun yaniti kisa yanittan ustun TUTMA.\n"
