# scripts/utils.py -- Sonuc kaydetme, Memory Bank guncelleme, seans sonu
...
def sonuclari_kaydet(iter_sonuc):
    ...
    try:
        ...
        print(f"[OK] GitHub: {mesaj}")
    except subprocess.CalledProcessError as e:
        stderr = e.stderr.decode() if e.stderr else ""
        if "nothing to commit" not in stderr:
            print(f"[WARN] Git: {stderr[:200]}")
...
"""
    ...
    if os.path.exists(progress_dosya):
        with open(progress_dosya, encoding="utf-8") as f:
            mevcut = f.read()
    if "## Ilerleme" not in mevcut:
        mevcut = "# progress.md\n\n## Ilerleme\n\n| Zaman | Asama | Deney | LiveBench |\n|---|---|---|---|\n"
...
def seans_sonu_kaydet(sonuclar, hf_repo_chat):
    if not sonuclar:
        print("[WARN] Kaydedilecek sonuc yok.")
        return
    son = sonuclar[-1]
    import glob
    ...
    if lora_yol:
        ...
        print("[WARN] Adapter bulunamadi.")
    sonuclari_kaydet(son)
    memory_guncelle(son)
    print("[OK] Seans sonu yedekleme tamamlandi.")
